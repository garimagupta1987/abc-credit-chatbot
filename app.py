...
QUESTIONS = [
    ...
    {"field": "Loan_Amt", "question": "Enter the loan amount you need (in Rs, between Rs 10,000 and Rs 10,00,000).", "type": "number"},
...
def validate(field, value, session_data=None, options=None):
    if field == "mobile":
        ...
        if not value.isdigit() or not (10000 <= int(value) <= 1000000):
            return False, "Loan amount must be between Rs 10,000 and Rs 10,00,000."
        ...
        if session_data and int(value) < session_data.get("Loan_Amt", 0):
            loan = session_data.get("Loan_Amt", 0)
            s = str(loan)
            last3 = s[-3:]
            rest = s[:-3]
            groups = []
            ...
            if rest:
                groups.append(rest)
            formatted = ",".join(reversed(groups)) + "," + last3 if rest or groups else last3
            return False, f"Asset value cannot be less than loan amount (Rs {formatted})."
...
return False, f"Asset value cannot be less than loan amount (Rs {indian_fmt(session_data.get('Loan_Amt', 0))})."
    elif field == "Net_Sal":
        if not value.isdigit() or int(value) < 1000:
            return False, "Please enter a valid salary of at least Rs 1,000."
    elif options:
